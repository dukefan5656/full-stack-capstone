...
import Search from "./SearchPageContainer";
...
describe("Search", () => {
  ...
  it("updates form state", () => {
    ...
    const expectedState = ["searchCity", "searchZip", "searchType"].reduce(
      (obj, key) => ({ ...obj, [key]: key }),
      {}
    );
  ...
  describe("handleSubmit", () => {
    ...
    it("updates state results", () => {
      ...
      search.mockReturnValue(
        Promise.resolve({
          json: () => Promise.resolve([{ _id: 1 }, { _id: 2 }, { _id: 3 }])
        })
      );
      ...
      return new Promise(resolve => setTimeout(resolve, 250)).then(() => {
        expect(wrapper.find(Search).state("results")).toEqual([
          { _id: 1 },
          { _id: 2 },
          { _id: 3 }
        ]);
