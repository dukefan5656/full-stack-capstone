...
describe("Seller", () => {
  it("renders without crashing", () => {
    withProvider(
      mount,
      mockStore(),
      <Seller getPayload={() => Promise.resolve()} bids={[]} listings={[]} />
    );
  });
  ...
  it("calls getPayload", () => {
    ...
    withProvider(
      mount,
      mockStore(),
      <Seller getPayload={getPayload} bids={[]} listings={[]} />
    );
  ...
  it("iterates over bids and listings", () => {
    ...
    const createBid = createObjCreator({
      user: { local: {} },
      listing: { user: {} }
    });
    const createListing = createObjCreator({
    ...
    const store = mockStore({
      ...
      entities: {
        listings: listings.reduce(
          (obj, listing) => ({ ...obj, [listing._id]: listing }),
          {}
        )
      }
    });
